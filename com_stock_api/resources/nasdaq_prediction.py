...
class PredGraph(Resource):

    @staticmethod
    def get():
        print("=====nasdaq_prediction.py / PredGraph's get")
    ...
    @staticmethod
    def post():
        print("=====nasdaq_prediction.py / PredGraph's post")
        ...
        service = PredicService()
        args = parser.parse_args()
        nasdaq = NasdaqPredictionVo()
        nasdaq.ticker = args.ticker
        nasdaq.open = args.open
        nasdaq.high = args.high
        nasdaq.low = args.low
        arr = np.zeros((2, 512*512), dtype=np.float32)
        arr[0, :] = nasdaq.high
        arr[1, :] = nasdaq.low
        print(nasdaq)
        # service.assign(nasdaq)
